...
const UserProfileCard = () => {
  ...
  const changePasswordHandler = async () => {
    ...
    if (password) {
      try {
        const { data, error: passError } = await supabase.auth.update({
          password,
        });
        if (passError) {
          throw new Error();
        ...
        <Modal centered opened={success} onClose={() => setSuccess(false)}>
          <div className="flex flex-1 flex-col justify-center items-center text-center">
            <AiOutlineCheckCircle
              size={75}
              className="text-emerald-700 mb-12"
            />
            <p className="font-semibold text-lg">
              Your Profile picture has been updated successfully! This make take
              a while to show up everywhere.
            </p>
          </div>
        </Modal>
        ...
        <div className="flex flex-1 flex-col justify-center p-10 items-center w-11/12 lg:w-2/6 md:w-4/6 border-2 rounded-lg shadow-md mx-auto my-12">
                ...
                <section className="p-10 border-2 border-dashed rounded-lg flex flex-1 justify-center items-center w-full mx-auto">
                  <div {...getRootProps()}>
                    ...
                    <p className="font-bold text-gray-500 text-center">
                      Drag and Drop or Click Here To Add A Photo!
                    </p>
                  </div>
                </section>
              )}
          ...
          {userContext.role === "admin" && (
            <Link href="/admin/admin-dashboard" passHref>
              <p className="font-semibold font-gray-500 cursor-pointer hover:underline">
          ...
          {userContext.googleUser && (
            <p className="text-sm text-center my-4 font-semibold">
              Your name was provided to us from your Google Account. You can get
              edit your name in your Google Account settings.
            </p>
          )}
        </div>
      </SectionHeading>
